...
export default class Toast {
  ...
  static success(textContent = '操作成功', duration = 2000) {
    Toast.clearOptions();
    Toast.setOptions({icon: icon_success});
    Toast.toast(textContent, duration);
  }

  static error(textContent = '操作成功', duration = 2000) {
    Toast.clearOptions();
    Toast.setOptions({icon: icon_error});
    Toast.toast(textContent, duration);
  }

  static show(textContent = '操作成功', duration = 2000) {
    Toast.clearOptions();
    Toast.setOptions({});
    Toast.toast(textContent, duration);
  }

  static toast(textContent = '未知信息', duration = 2000) {
    // toast蒙层默认透明
  ...
  static restoreRoot(animated: boolean, animatesOnly = false) {
